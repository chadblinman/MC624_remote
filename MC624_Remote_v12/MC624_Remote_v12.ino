...
//  Serial.begin(9600);
  ...
  delay(100);
  for (int blinky = 0; blinky <= 14; blinky++)   // and off
    {
    digitalWrite(marquee_button[blinky], LOW);
    }      
  delay(100);
  for (int blinky = 0; blinky <= 14; blinky++)   // and on again
    {
    digitalWrite(marquee_button[blinky], HIGH);
    }
  delay(100);
  for (int blinky = 0; blinky <= 14; blinky++)   // and off again
    {
    digitalWrite(marquee_button[blinky], LOW);
    }      
  delay(100);
...
//    Serial.println(functions, BIN);        // print to serial monitor 
  ...
  NumberArray[0] = (inSelect + 1);         // selected input (source)
