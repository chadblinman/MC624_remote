...
int dim_button = 28; int dim_LED = 43;
int mono_button = 29; int mono_LED = 44;
int invR_button = 30; int invR_LED = 45;
int muteL_button = 31; int muteL_LED = 46;
int muteR_button = 32; int muteR_LED = 47;
int spk4_button = 33; int spk4_LED = 48;
int spk3_button = 34; int spk3_LED = 49;
int spk2_button = 35; int spk2_LED = 50;
int spk1_button = 36; int spk1_LED = 51;
...
int volume = 0;                     // the volume level sent to MC624
int displayVolume = 0;              // volume formatted for Nixie display
...
float fadeMax = 8.0f;
float fadeStep = 1.0f;           // Adjust for fade effect.  Smaller number = slower fade.
...
//  Serial.begin(9600);
  ...
  // Run button marquee effect:
  for (int marquee = in1_LED; marquee <= spk1_LED; marquee++)   // Left to right
  {
    digitalWrite(marquee, HIGH);
    delay(60);
    digitalWrite(marquee, LOW);
  }
  for (int marquee = spk2_LED; marquee >= in1_LED; marquee--)   // Right to left
  {
    digitalWrite(marquee, HIGH);
    delay(60);
    digitalWrite(marquee, LOW);
...
//  delay(20);
  ...
  // Fill in the Number array for the Nixie tubes and display:
  NumberArray[0] = (inSelect + 1);         // Selected input
  NumberArray[2] = volumeTens;             // Current volume - tens digit
  NumberArray[3] = volumeOnes;             // Current volume - ones digit
  ...
  DisplayFadeNumberString();               // Display

  // if the analog value has changed, write to RS485 serial:
